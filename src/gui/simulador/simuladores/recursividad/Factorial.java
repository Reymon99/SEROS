...
public final class Factorial extends Recursividad {
    ...
    protected void casoTerminal() {
        base(Text.FACTORIAL_1,
                ...
                Operaciones.productUpFactorial(Integer.parseInt(valorDato.getValue().toString()), getValor()), true);
    }
    @Override
    protected void casoDecrementativo() {
        base(Text.CASO_RECURSIVO, 0,
                Operaciones.productoFactorial(Integer.parseInt(valorDato.getValue().toString()), getValor()), false);
        incrementIteracion();
    }
    @Override
    protected void casoIncrementativo() {
        base(Text.FACTORIAL_2,
                Operaciones.formatNumber(Operaciones.factorial(getValor()), Operaciones.Operacion.FACTORIAL),
                Operaciones.productUpFactorial(Integer.parseInt(valorDato.getValue().toString()), getValor()), false);
        decrementIteracion();
    }
    @Override
    protected void casoBase(boolean found) {
        base(found ? Text.FACTORIAL_1 : Text.CASO_BASE_RETURN_2,
                ...
                Operaciones.productoFactorial(Integer.parseInt(valorDato.getValue().toString())), found);
        if (!found) {
            decrementIteracion();
            setDecremento(false);
        }
    ...
    protected Lines[] lines() {
        ...
        return new Lines[]{
                new Lines(this, new LineLocation(0, 5, Text.CASO_RECURSIVO_FINISHED.toString(), false)) {
                    @Override
                ...
                }, new Lines(this, lines[0], lines[1],
                        new LineLocation(0, 4, Text.CASO_RECURSIVO.toString()), lines[2]) {
                    @Override
                    protected void actions() {
    ...
    public void iteracion0() {
        Eventos.enable(false,getNextIteracion(), getSend(), valorDato, getPause(), getBack(), getClean(), getCodigo(), getHome());
        Eventos.variable(variableN,-1, valorDato.getValue());
        base(Text.FACTORIAL_1,
                Operaciones.formatNumber(Operaciones.factorial(Integer.parseInt(valorDato.getValue().toString())), Operaciones.Operacion.FACTORIAL),
                Operaciones.productoFactorial(Integer.parseInt(valorDato.getValue().toString())), true);
    }
    @Override
